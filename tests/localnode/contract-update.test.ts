...
import { ToString, LocalTestnetAddresses, expectIsError } from "../utils";
...
describe("Contract terms update", function () {
  ...
  before(async () => {
    ...
    await lumerin.methods
      .increaseAllowance(cloneFactoryAddress, ToString(10 * 10 ** 8))
      .send({ from: buyer });
  ...
  it("should create contract and check its status", async function () {
    const receipt = await cf.methods
      .setCreateNewRentalContractV2(price, "0", "1", "3600", "0", cloneFactoryAddress, "123")
  ...
  it("should prohibit updating if caller is not a seller", async function () {
    try {
      await cf.methods
        .setUpdateContractInformation(hrContractAddr, newPrice, "1", "1", "1")
        .send({ from: buyer });
  ...
  it("should update contract and emit event without futureTerms update", async function () {
    const receipt = await cf.methods
      .setUpdateContractInformation(hrContractAddr, newPrice, "2", "3", "4")
      .send({ from: seller });
    ...
    const events = await impl.getPastEvents("purchaseInfoUpdated", {
      fromBlock: receipt.blockNumber,
      toBlock: "latest",
    });
    const isEventFound = events.find((e) => e.returnValues._address === hrContractAddr);
  ...
  it("should store futureTerms for contract and should not emit update event if contract is running", async function () {
    ...
    await cf.methods
      .setPurchaseRentalContract(hrContractAddr, "", "1")
      .send({ from: buyer, value: fee });
    const receipt = await cf.methods
      .setUpdateContractInformation(hrContractAddr, newPrice, "22", "33", "44")
      .send({ from: seller });
    const impl = Implementation(web3, hrContractAddr);
    const futureTerms = await impl.methods.futureTerms().call();
    const data = await impl.methods.getPublicVariablesV2().call();
  ...
  it("should set profitTarget", async function () {
    await cf.methods
      .setUpdateContractInformationV2(hrContractAddr, newPrice, "2", "3", "4", 1)
      .send({ from: seller });
    const impl = Implementation(web3, hrContractAddr);
    const data = await impl.methods.getPublicVariablesV2().call();
    expect(data._terms._profitTarget).equal("1");
  });
});
