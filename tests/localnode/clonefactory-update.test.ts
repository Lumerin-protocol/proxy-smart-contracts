...
describe("Clonefactory update", function () {
  ...
  const validatorFeeRate = 0.01;

  web3.eth.accounts.wallet.create(0).add(web3.eth.accounts.privateKeyToAccount(deployerPrivateKey));
  ...
  it("should deploy a new clonefactory", async () => {
    ...
    ({ address: cloneFactoryAddr } = await DeployCloneFactory(
      lumerinAddr,
      deployerPrivateKey,
      feeRecipientAddress,
      validatorFeeRate,
