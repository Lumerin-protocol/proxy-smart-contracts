...
describe("Contract closeout", function () {
  ...
  it("should disallow closeout type 0 twice", async function () {
    const receipt = await cf.methods.setCreateNewRentalContract(price, "0", speed, length, "0", cloneFactoryAddress, "123").send({ from: seller, value: fee })
...
async function testEarlyCloseout(progress, fee, seller, buyer, cloneFactoryAddress, lumerinAddress, web3) {
  ...
  const receipt = await cf.methods.setCreateNewRentalContract(price, "0", speed, String(length), "0", cloneFactoryAddress, "123").send({ from: seller, value: fee })
  const hrContractAddr = receipt.events?.contractCreated.returnValues._address;
