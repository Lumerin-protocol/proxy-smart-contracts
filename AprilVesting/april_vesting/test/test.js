let { expect } = require("chai");
let { ethers } = require("hardhat");
let scripts = require('../scripts/main.js')


describe("ContractPurchase", function () {
	this.timeout(600*1000);
	//function to create a lumerin and tokenDrop at the designated addresses
	let lumerin
	let tokenDrop
	let accounts
	let addresses
	let values
	let initialProvider
	let tx_amt = 1000
	beforeEach(async function() {
		/*
		 * deployment and initialization of contracts (lumerin and token drop
		 * the plan is to remove the deployment once all other aspects are sucessful
		 * and test against contracts deployed on the following networks
		 * localhost
		 * ropsten
		 * mattic
		 */
		initialProvider = await ethers.getSigner()
		defaultProvider = initialProvider.provider
		const Lumerin = await ethers.getContractFactory("Lumerin");
		lumerin = await Lumerin.deploy()
		await lumerin.deployed()
		const FastLumerinDrop = await ethers.getContractFactory("FastLumerinDrop");
		tokenDrop = await FastLumerinDrop.deploy(lumerin.address)
		await tokenDrop.deployed()
		//used to test bulk import of values
		tokenDropMass = await FastLumerinDrop.deploy(lumerin.address)
		await tokenDropMass.deployed()
		console.log('lumerin deployed to: ', lumerin.address)
		console.log('fast token drop deployed to: ', tokenDrop.address)
		console.log('fast token drop mass deployed to: ', tokenDrop.address)
		await lumerin.transfer(tokenDrop.address, tx_amt)
		await lumerin.transfer(tokenDropMass.address, tx_amt)
		let fastDropBalance = await lumerin.balanceOf(tokenDrop.address)
		expect(String(fastDropBalance)).to.equal(String(tx_amt))

		/*
		 * obtaining the csv file where each row contains
		 * an address, private key, and lumerin vesting value
		 * this is important to have the address and private key
		 * since testing will also include retrieving funds
		 */

		/*
		 * creating a temp accounts object to use to populate
		 * the fast drop contract. this logic will be replaced
		 * with a csv file intake
		 */
		accounts = []
		addresses = []
		values = []
		let wallet1 = ethers.Wallet.createRandom()
		let wallet2 = ethers.Wallet.createRandom()
		let wallet3 = ethers.Wallet.createRandom()
		let wallet4 = ethers.Wallet.createRandom()
		wallet1 = wallet1.connect(defaultProvider)
		wallet2 = wallet2.connect(defaultProvider)
		wallet3 = wallet3.connect(defaultProvider)
		wallet4 = wallet4.connect(defaultProvider)
		accounts.push([wallet1.address, wallet1.privateKey, 100, wallet1])
		accounts.push([wallet2.address, wallet2.privateKey, 200, wallet2])
		accounts.push([wallet3.address, wallet3.privateKey, 300, wallet3])
		accounts.push([wallet4.address, wallet4.privateKey, 0, wallet4])

		/*
		 * inputting the values into the fast drop contract
		 * first step is to use the generated accounts file
		 * also sending them 1 eth
		 * later on intake from csv file
		 */
		for (let account of accounts) {
			await scripts.addIndividualAddressToFastTokenDrop(
				account[0], 
				account[2], 
				tokenDrop
			)
			let eth_tx = {
				to: account[0],
				value: ethers.utils.parseEther('1')
			}
			await initialProvider.sendTransaction(eth_tx)
			addresses.push(account[0])
			values.push(account[2])
			
		}
		//adding larger groups into the token drop
		await scripts.addMultipleAddressesToFastTokenDrop(addresses, values, tokenDropMass)
	})

	it("check to see if balance can be claimed on tokenDrop", async function () {
		/*
		 * go through list of accounts
		 * call the contract claim function for each account
		 * confirm that the balance of each account is now
		 * the same as what is in the list
		 */
		for (let account of accounts) {
			let user = account[3]
			//call claim function
			await tokenDrop.connect(user).Claim()
			//check balance of token
			let userBalance = await lumerin.balanceOf(account[0])
			expect(String(userBalance)).to.equal(String(account[2]))
		}
	});
	it("check to see if claiming twice returns twice the funds on tokenDrop", async function () {
		for (let account of accounts) {
			let user = account[3]
			//call claim function twice to simulate a double claim
			await tokenDrop.connect(user).Claim()
			await tokenDrop.connect(user).Claim()
			//check balance of token
			let userBalance = await lumerin.balanceOf(account[0])
			expect(String(userBalance)).to.equal(String(account[2]))
		}
	});
	it("check to see if address without claiming can still claim on tokenDrop", async function () {
			let account = accounts[3]
			let user = account[3]
			//call claim function twice to simulate a double claim
			await tokenDrop.connect(user).Claim()
			await tokenDrop.connect(user).Claim()
			//check balance of token
			let userBalance = await lumerin.balanceOf(account[0])
			expect(String(userBalance)).to.equal(String(account[2]))
	});
	it("check to see if balance can be claimed on tokenDropMass", async function () {
		/*
		 * go through list of accounts
		 * call the contract claim function for each account
		 * confirm that the balance of each account is now
		 * the same as what is in the list
		 */
		for (let account of accounts) {
			let user = account[3]
			//call claim function
			await tokenDropMass.connect(user).Claim()
			//check balance of token
			let userBalance = await lumerin.balanceOf(account[0])
			expect(String(userBalance)).to.equal(String(account[2]))
		}
	});
	it("check to see if claiming twice returns twice the funds on tokenDropMass", async function () {
		for (let account of accounts) {
			let user = account[3]
			//call claim function twice to simulate a double claim
			await tokenDropMass.connect(user).Claim()
			await tokenDropMass.connect(user).Claim()
			//check balance of token
			let userBalance = await lumerin.balanceOf(account[0])
			expect(String(userBalance)).to.equal(String(account[2]))
		}
	});
	it("check to see if address without claiming can still claim on tokenDropMass", async function () {
			let account = accounts[3]
			let user = account[3]
			//call claim function twice to simulate a double claim
			await tokenDropMass.connect(user).Claim()
			await tokenDropMass.connect(user).Claim()
			//check balance of token
			let userBalance = await lumerin.balanceOf(account[0])
			expect(String(userBalance)).to.equal(String(account[2]))
	});
	it("confirm lumerin can be sent from fastdrop to elsewhere", async function () {
		account3Address = accounts[3][0]
			let fastDropBefore = await lumerin.balanceOf(tokenDrop.address)
			let account3Before = await lumerin.balanceOf(account3Address)
			expect(String(fastDropBefore)).to.equal(String(tx_amt))
			expect(String(account3Before)).to.equal(String(0))
			await tokenDrop.TransferLumerin(account3Address, tx_amt)
			let fastDropAfter = await lumerin.balanceOf(tokenDrop.address)
			let account3After = await lumerin.balanceOf(account3Address)
			expect(String(fastDropAfter)).to.equal(String(0))
			expect(String(account3After)).to.equal(String(tx_amt))
	});
});
